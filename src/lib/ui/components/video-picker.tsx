...
import { useFileUpload } from "@/lib/ui/hooks/use-file-upload";
...
export default function VideoPicker({
  ...
  useEffect(() => {
    if (files.length > 0 && setFileAction) {
      setFileAction(files[0].file as File);
    }
  }, [files, setFileAction]);
        ...
        >
          ...
          {previewUrl ? (
            <div className="absolute inset-0">
              <video src={previewUrl} className="size-full object-contain" controls {...props} />
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center px-4 py-3 text-center">
              <p className="text-muted-foreground text-sm mb-2.5">
                {placeholder || "Arrastra y suelta una imagen aquí"}
              </p>
              <div
              ...
              <p className="mb-1.5 text-sm font-medium">
                Suelta tu video aquí o haz clic para buscar
              </p>
